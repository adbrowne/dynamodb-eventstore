...
writeToDynamoAws
  DynamoKey { dynamoKeyKey = streamId, dynamoKeyEventNumber = eventNumber}
  values
  version = 
  catches writeItem [handler _ConditionalCheckFailedException (\_ -> return DynamoWriteWrongVersion)]
  where
...
queryTableAws :: QueryDirection -> Text -> Natural -> Maybe Int64 -> MyAwsM [DynamoReadResult]
queryTableAws direction streamId limit exclusiveStartKey =
  getBackward
    where
...
readFieldGeneric :: (MonadError e m) => (Text -> e) -> Text -> Lens' AttributeValue (Maybe a) -> DynamoValues -> m a
