...
writeToDynamoAws :: DynamoKey -> DynamoValues -> DynamoVersion -> MyAwsM DynamoWriteResult
writeToDynamoAws DynamoKey {dynamoKeyKey = streamId
                           ,dynamoKeyEventNumber = eventNumber} values version =
  catches
    writeItem
    [ handler
        _ConditionalCheckFailedException
        (\_ -> return DynamoWriteWrongVersion)]
  where
...
updateItemAws DynamoKey {dynamoKeyKey = streamId
                        ,dynamoKeyEventNumber = eventNumber} values = go
  where
    ...
    go = do
      ...
      let req0 =
            ...
            if (not . HM.null) expressionAttributeValues
              then set uiExpressionAttributeValues expressionAttributeValues
              else id
    ...
    setStartKey (Just startKey) =
      set
        ...
        (HM.fromList
           ...
           , ("eventNumber", set avN (Just $ show startKey) attributeValue)])
    ...
    getBackward = do
      ...
      resp <-
        ...
        set
          ...
          (HM.fromList [(":streamId", set avS (Just streamId) attributeValue)]) &
...
runLocalDynamo runtimeEnvironment x = do
  ...
  runResourceT $
    runAWST runtimeEnvironment' $ reconfigure dynamo $ runExceptT (unMyAwsM x)
