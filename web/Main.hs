...
buildActionRunner
  :: MonadEsDsl m
  => (forall a. m a -> ExceptT InterpreterError IO a) -> EventStoreActionRunner
buildActionRunner runner =
  EventStoreActionRunner
  { eventStoreActionRunnerPostEvent =
    (\req ->
        liftIO $
        runExceptT $
        (PostEventResult <$> runWithState (postEventRequestProgram req)))
  , eventStoreActionRunnerReadEvent =
    (\req ->
        liftIO $
        runExceptT $
        (ReadEventResult <$> runWithState (getReadEventRequestProgram req)))
  , eventStoreActionRunnerReadStream =
    (\req ->
        liftIO $
        runExceptT $
        (ReadStreamResult <$> runWithState (getReadStreamRequestProgram req)))
  , eventStoreActionRunnerReadAll =
    (\req ->
        liftIO $
        runExceptT $
        (ReadAllResult <$> runWithState (getReadAllRequestProgram req)))
  }
  where
...
start parsedConfig = do
  ...
  when
    ...
    (putStrLn "Creating table..." >>
     toApplicationError (interperter runtimeEnvironment) (buildTable tableName) >>
