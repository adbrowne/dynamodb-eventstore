...
getEntry isEmbedTryHarder url = do
  let opts =
        defaults & header "Accept" .~ ["application/vnd.eventstore.atom+json"] &
        auth ?~
        basicAuth "admin" "changeit"
  let getUrl =
        if isEmbedTryHarder
...
getRelLinks relName =
  ...
  toListOf
    (html .
     allNamed (only "link") . attributed (ix "rel" . only relName) . attr "href")
...
config =
  ...
  Opt.subparser
    (Opt.command
     ...
     Opt.command
       "copyGlobalStream"
       (Opt.info copyGlobalStreamOptions (Opt.progDesc "Copy global stream")) <>
     Opt.command
       "compareDownload"
       (Opt.info
     ...
     Opt.command
       "speedTest"
       (Opt.info speedTestOptions (Opt.progDesc "Test bulk creation of pages")) <>
     Opt.command
       "insertData"
       (Opt.info insertDataOptions (Opt.progDesc "Insert data into event store")))
  where
    ...
    compareDownloadOptions =
      ...
      (CompareDownloadConfig <$>
       Opt.strOption
         (Opt.long "referenceDirectory" <> Opt.metavar "REFERENCE_DIRECTORY" <>
...
compareFeedFiles leftFilePath rightFilePath = do
  ...
  unless areEqual $
    do putStrLn
         ("Left: " <> T.pack leftFilePath <> " Right: " <> T.pack rightFilePath)
       let theDiff =
             getGroupedDiff (domToTextLines domLeft) (domToTextLines domRight)
...
listDirectoryTree path = do
  ...
  childrenWithIsDir <- 
    mapM
      (\a ->
          doesDirectoryExist (prependPath a) >>= (\isDir -> return (isDir, a)))
      directoryChildren
...
makeTestEntry n =
  ...
  , entryDataBody =
    Just . TL.encodeUtf8 . TL.fromStrict $ "{ \"a\":" <> show n <> "}"
  }
...
start Config {configCommand = DownloadGlobalStream DownloadGlobalStreamConfig {..}} = do
  ...
  void $
    sequence $ outputResponse downloadGlobalStreamOutputDirectory "next" <$> numberedResponses
...
start Config {configCommand = DownloadFeed DownloadFeedConfig {..}} = do
  ...
  let numberedResponses = zip [0 ..] responses
  void $
    sequence $ outputResponse downloadFeedConfigOutputDirectory "next" <$> numberedResponses
